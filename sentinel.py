...
def call_ai(incident: dict) -> dict:
    ...
    if not api_key:
        return {
            ...
            "confidence": 95,
            "reasoning": (
                f"Patrón de reconocimiento activo: {incident['hits']} conexiones "
                f"en {incident['window_ms']}ms a servicios {incident['services']}. "
                "Comportamiento consistente con escaneo Nmap. Bloqueo recomendado."
            )
        }
    try:
        ...
        system_prompt = """Eres el motor de análisis de un SOC (Centro de Operaciones de Seguridad).
Recibes incidentes detectados por agentes honeypot.
Responde SOLO con JSON válido: {"action":"BLOCK","confidence":95,"reasoning":"análisis en español, 2 frases"}
Sin texto adicional. Sin markdown."""

        user_msg = (
            f"INCIDENTE:\nIP: {incident['ip']} | "
            f"Servicios: {incident['services']} | "
            f"Hits: {incident['hits']} en {incident['window_ms']}ms | "
            f"Riesgo: ALTO"
        )
        ...
        logger.warning("API IA no disponible: %s — usando fallback.", e)
        return {
            "action": "BLOCK",
            "confidence": 90,
            "reasoning": f"API no disponible. Bloqueo preventivo aplicado por seguridad ante {incident['hits']} hits detectados."
        }
